...
namespace ZeroElectric.Vinculum.Examples.Physics;
...
public unsafe static class PhysicsDemo
{
	...
	public static int main()
	{
		...
		PhysicsBodyData* floor = CreatePhysicsBodyRectangle(new Vector2(screenWidth / 2.0f, (float)screenHeight), 500, 100, 10);
		...
		while (!WindowShouldClose())    // Detect window close button or ESC key
		{
			...
			if (IsKeyPressed(KEY_R))    // Reset physics system
			{
				...
				floor = CreatePhysicsBodyRectangle(new Vector2(screenWidth / 2.0f, (float)screenHeight), 500, 100, 10);
				floor->enabled = false;
